  ...
  return (
    <div ref={box} className="relative">
      ...
      {open && results.length > 0 && (
        <ul
          className={`absolute z-30 mt-1 max-h-72 w-[min(18rem,calc(100vw-1rem))] min-w-full divide-y divide-slate-100 overflow-auto rounded-md border border-slate-200 bg-white shadow-lg ${
            ...
            <li key={c.code}>
              ...
              >
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className={`text-sm font-semibold ${c.dormant ? 'text-red-600' : ''}`}>
                    {c.display_code}
                  </span>
                  {c.ge_codes.map((g) => (
                  ...
                  {c.dormant && (
                    <span className="whitespace-nowrap rounded bg-red-100 px-1 text-[10px] font-medium text-red-700">
                      not offered in 5+ years
                    </span>
                  )}
                </div>
                <div className="mt-0.5 text-xs leading-snug text-slate-600">{c.title}</div>
              </button>
            </li>
          ))}
        </ul>
