  ...
  return (
    <div ref={box} className="relative">
      <input
        ...
        className="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-1.5 text-sm shadow-sm focus:border-sky-500 focus:outline-none"
      />
      {open && results.length > 0 && (
        <ul
          className={`absolute z-30 mt-1 max-h-72 w-[min(18rem,calc(100vw-1rem))] min-w-full divide-y divide-slate-100 dark:divide-slate-800 overflow-auto rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-lg ${
        ...
        >
          {results.map((c) => (
            <li key={c.code}>
              <button
                className="w-full px-3 py-2 text-left hover:bg-sky-50 dark:hover:bg-slate-800"
                onClick={() => {
                  onSelect(c)
                  setQ('')
                  setOpen(false)
              ...
              >
                ...
                <div className="mt-0.5 text-xs leading-snug text-slate-600 dark:text-slate-400">{c.title}</div>
              </button>
            </li>
          ))}
        </ul>
